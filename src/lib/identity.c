...
static int Identity_load_certs(Identity self, char *cert, char *priv_key) {
  ...
  if(priv_key) {
    fd = (FileLikeObject)CALL(oracle, open, NULL, priv_key, 'r');
    if(fd && fd->size < 1e6) {
  ...
  if(cert) {
    fd = (FileLikeObject)CALL(oracle, open, NULL, cert, 'r');
    if(fd && fd->size < 1e6) {
      char *text = talloc_size(fd, fd->size);
      BIO *mem;
...
static void Identity_verify(Identity self) {
  ...
  for(statement_urn=statements; *statement_urn; statement_urn++) {
    ...
    FileLikeObject statement = (FileLikeObject)CALL(oracle, open, NULL, fq_name,'r');
    ...
    oracle_signature = (FileLikeObject)CALL(oracle, open, NULL, buff,'r');
    if(!oracle_signature) {
