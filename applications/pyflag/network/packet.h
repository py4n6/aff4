...
#define INIT_STRUCT(struct_member_name, struct_format)				\
  ((Packet)self)->struct_p = (void *)((char *)&(self->struct_member_name) \
				      - (char *)(self));		\
  ((Packet)self)->format = struct_format;				\
  INIT_LIST_HEAD(&((Packet)self)->properties.list);
...
#define __NAME_ACCESS_start(struct_member_name, member, ref_name, type)	\
  do {									\
    ...
    p->name=#ref_name;							\
    p->field_type = type;						\
    p->item = (typeof(p->item))(&((typeof(self->struct_member_name) *)NULL)->member); \
    p->size = sizeof(self->struct_member_name.member);

#define __NAME_ACCESS_end(struct_member_name, member, name, type)	\
  list_add_tail(&(p->list), &((Packet)self)->properties.list);		\
  } while(0)
...
#define NAME_ACCESS_SIZE(struct_member_name, member, name, type, size_member) \
  __NAME_ACCESS_start(struct_member_name, member, name, type);		\
  p->size_p = (typeof(p->size_p))(&((typeof(self->struct_member_name) *)0)->size_member); \
  p->size = 0;								\
  __NAME_ACCESS_end(struct_member_name, member, name, type);		  
