...
TCPHashTable TCPHashTable_Con(TCPHashTable self, int initial_con_id) {
  ...
  for(i=0;i<TCP_STREAM_TABLE_SIZE; i++) {
    self->table[i] = talloc(self, struct TCPStream_t);
    INIT_LIST_HEAD(&(self->table[i]->list));
  };

  // This list keeps all streams in order:
  self->sorted = talloc(self, struct TCPStream_t);
...
TCPStream TCPHashTable_find_stream(TCPHashTable self, IP ip) {
  ...
  */
  ...
  if(udp_packet) {
    i = (TCPStream)CONSTRUCT(UDPStream, TCPStream, Con, self, &forward, self->con_id++);
  } else {
  ...
  if(udp_packet) {
    j = (TCPStream)CONSTRUCT(UDPStream, TCPStream, Con, i, &reverse, self->con_id++);
  } else {
