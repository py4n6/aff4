...
class HTMLUI:
    ...
    def __init__(self,default = None,query=None, initial=None):
        ...
        if default != None:
            ...
            self.form_parms =Framework.Query(())
            self.form_target = 'self'
            self.defaults = Framework.Query(())
...
## parent they will know the correct js window to go to.        
    ...
    def heading(self,string):
        ...
        self.result += expand("<h1>%s</h1>", string)
    ...
    def opt_to_str(self,opts={}, **options):
        ...
        for k,v in options.items():
            if v:
                result.append(expand("%s=%r", (k,quote_quotes(unicode(v)))))
    ...
    def tree(self, tree_cb = None, pane_cb=None, branch = None, layout=None):
        ...
        def draw_branch(depth,preamble,query):
            ...
            try:
            ## Get the right part:
                branch=Framework.splitpath(query['open_tree'])
            except KeyError:
                branch=['']
            ...
            def render_row(name, value, state, preamble):
                ...
                link['open_tree'] = Framework.normpath("/".join(branch[:depth] + [name]))
                ...
                result+="<a href=\"javascript:tree_open('%s','%s','f?%s')\"><img class=%r src='%s'>" % (cb,query['right_pane_cb'], Framework.iri_to_inline_js(link), img_class, img_src)
        ...
        def right(query,result):
            ...
            try:
            ## Get the right part:
                path=Framework.normpath(query['open_tree'])
            except KeyError:
                path=''
    ...
    def end_form(self,value='Submit',name='__submit__',**opts):
        ...
        for k,v in self.form_parms:
            if not k.startswith("__"):
                base += expand("<input type=hidden name='%s' value=\"%s\">\n", (k,unicode(cgi.escape(v, True))))
