...
if len(sys.argv) != 2:
        print "Usage: %s registry_file" % sys.argv[0]
        sys.exit(-1)

REGISTRY_FILE = pyregfi.RegistryFile(sys.argv[1])
...
class Directory:
	...
	def __init__(self, path):
		...
		try:
                        ...
                        iter = REGISTRY_FILE.get_key(components)
                        files = [ x.rec.valuename for x in iter.list_values()]
                        dirs = [ k.keyname for k in iter ]
