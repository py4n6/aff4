...
static StringIO StringIO_constructor(StringIO self) {
  ...
  self->data = talloc_size(self,1024);
...
static int StringIO_write(StringIO self,char *data, unsigned int len) {
  if(self->readptr+len >= self->alloc_size) {
    self->alloc_size += self->readptr + len;
    self->data = talloc_realloc_size(self,self->data,self->alloc_size);
  };
